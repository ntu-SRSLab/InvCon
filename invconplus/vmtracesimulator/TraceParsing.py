...
class Simulator:
    ...
    def __init__(self, address, tx_hash) -> None:
        ...
        if os.path.exists(f"{CACHE_DIR}/txs/{tx_hash}.json"):
            ...
            json.dump(self.data, open(f"{CACHE_DIR}/txs/{tx_hash}.json","w"), indent=4)
    ...
    def query_contract_calls(self):
        ...
        json.dump([call.to_json() for call in calls], open(f"{CACHE_DIR}/txs/{self.tx_hash}-{self.address}.calls.json", "w"),  indent=4)
        return calls 
