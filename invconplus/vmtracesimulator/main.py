...
class Simulator:
    ...
    contract_name: str 
    def __init__(self, tx_hash, contract_name = None) -> None:
        self.tx_hash = tx_hash 
        self.contract_name = contract_name
        logging.debug("analyzing transaction#{0}".format(tx_hash))
        if not os.path.exists(f"{CACHE_DIR}/txs"):
            os.mkdir(f"{CACHE_DIR}/txs")
        if os.path.exists(f"{CACHE_DIR}/txs/{tx_hash}.json"):
            self.data = json.loads(open(f"{CACHE_DIR}/txs/{tx_hash}.json", "r").read())
        else:
            self.data = fetchAllRuntimeInformation(tx_hash=tx_hash)

        if os.path.exists(f"{CACHE_DIR}/txs/{tx_hash}.query.json"):
            self.queries_cache = json.loads(open(f"{CACHE_DIR}/txs/{tx_hash}.query.json", "r").read())
        else:
    ...
    def query(self, slot):
        ...
        if slot in self.queries_cache:
            return self.queries_cache[slot]
        ...
        # dump the query results in to the files 

        self.queries_cache[slot] = list(set(result))

        json.dump(self.queries_cache, open(f"{CACHE_DIR}/txs/{self.tx_hash}.query.json", "w"), indent=4)  
...
def fetchSlotCalculationConstants(address:str, tx_hash: str) -> list:
    if not os.path.exists(f"{CACHE_DIR}/txs"):
        os.mkdir(f"{CACHE_DIR}/txs")
    if os.path.exists(f"{CACHE_DIR}/txs/{tx_hash}.json"):
        data = json.loads(open(f"{CACHE_DIR}/txs/{tx_hash}.json", "r").read())
        if data is None:
            return []
        ...
        try:
            ...
            if data is None:
                return []
            open(f"{CACHE_DIR}/txs/{tx_hash}.json", "w").write(json.dumps(data, indent=4))
        except:
            return []
