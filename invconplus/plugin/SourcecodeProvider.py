...
def compute_type_info(vartype, _type_info, contract):
    ...
    elif type_ in ["uint" + str(i*8) for i in range(1, 33)] + ["int" + str(i*8) for i in range(1, 33)]:
        _type_info[contract.name][type_]  =  dict(encoding="inplace", label=type_, numberOfBytes = str(vartype.storage_size[0]))
    elif type_ == "address":
