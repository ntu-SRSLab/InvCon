...
class Event:
    ...
    def getParameterValue(event, name, item_range=None, value_range=None, isArray=False):
                ...
                if m is not None:
                    ...
                    if m is not None:
                        ...
                        m  = re.match(r"ori\(size\((.*)\)\)", name)
                        if m is not None:
                            baseName = m.group(1)
                            src_var = "ori({})".format(baseName)
                            collectionValue = event.getParameterValue(name=src_var)
                            if isinstance(collectionValue, MappingVariableModel):
                                size = len(collectionValue)
                            else:
                                size = None 
                                if isinstance(collectionValue, list):
                                    if isinstance(collectionValue[0], StructVariableModel):
                                        if isinstance(collectionValue[0].varValue, list):
                                            if isinstance(collectionValue[0].varValue[0], ArrayVariableModel):
                                                size = len(collectionValue[0].varValue[0].varValue) 
                                if size is None:
                                    assert False, f"parameter {name} is not supported"
                            if item_range is None or value_range is None:
                                return size
                            elif item_range is not None:
                                start, end = item_range
                                if isinstance(end, str):
                                    m = re.match(r"(\w+)\s\+\s+([0-9]+)", name)
                                    if m is not None:
                                        end = event.getParameterValue(name=m.group(1)) + int(m.group(2))
                                    else:
                                        end = event.getParameterValue(name=end)
                                results = []
                                for i in range(start, end):
                                    results.append(size + i)
                                return results 
                            else:
                                assert False, f"parameter {name} is not supported"    
                        else:
    ...
    def computeState(self, statevar):
        ...
        for parameter in self.getParameters():
            if parameter.getName() == pure_statevar if not isOrignal else \
                ...
                if isinstance(parameter.getValue(), dict):
                    ...
                    target = parameter
                    if isinstance(target.value, list) and isinstance(target.value[0], StructVariableModel):
                            if isinstance(target.value[0].varValue[0], ArrayVariableModel) and isinstance(target.value[0].varValue[1], MappingVariableModel):
                                bytes_index = f"{int(index):#0{66}x}"
                                if bytes_index not in target.value[0].varValue[1].varValue:
                                    target = target.value[0].varValue[0].item_var_type.members()[1] 
                                else:
                                    keyIndex = target.value[0].varValue[1].varValue[bytes_index].varValue 
                                    target = target.value[0].varValue[0][keyIndex].varValue[1]
                    else:    
        ...
        if isinstance(target, StructVariableModel):
            ...
            return dict(name = varName if not isOrignal else f"ori({varName})", value = target.getValue(), vartype = target.varType)
...
class TraceSlice:
    ...
    def onlineSlice(self, event: Event):
        ...
        for theta in itertools.product(*self.global_parameter_binds.values()):
            parameter_binds = dict(zip(abstractTypes, theta)) # 1571, 1588
