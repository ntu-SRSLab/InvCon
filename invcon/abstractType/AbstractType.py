                ...
                if var.is_constant:
                    if isinstance(var.expression, Literal):
                        if var.expression.type.name in ["int"+str(8*i) for i in range(1,33)] + ["uint"+str(8*i) for i in range(1,33)]:
                            if var.expression.subdenomination == "ether":
                                value = int(float(var.expression.value)*(10**18))
                            elif var.expression.subdenomination == "weeks":
                                value = int(var.expression.value) * 7 * 24 * 3600
                            elif var.expression.subdenomination == "days":
                                value = int(var.expression.value) * 24 * 3600
                            elif var.expression.subdenomination == "hours":
                                value = int(var.expression.value) * 3600
                            elif var.expression.subdenomination == "minutes":
                                value = int(var.expression.value) * 3600
                            else:
    ...
    def createFunctionDataDependency(self, func: FunctionContract):
        ...
        def exploreEx(exp: Expression):
            if isinstance(exp, AssignmentOperation):
                ...
                elif member != "length" and isinstance(structVar, VarInfo) and StructMember.valid_vars([structVar]):
                ...
                if isinstance(val, EnumContract):
                    return val 
                elif isinstance(val, Contract):
                    return VarInfo(name=val.name, type="address", vartype= VarType.STATEVAR if val in self.contract.state_variables  else VarType.TXVAR if val in func.parameters or val.name in ["msg.sender", "msg.value"] else VarType.LOCALVAR, derivation=None)
                else:
