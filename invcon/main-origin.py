    ...
    if configuration is None:
        while (not txreplayer.done()) and count < maxCount:
            ...
            if not firstOrMultiple:
                ...
                if test_mode:
                    result_tests[tx.tx_hash]=dict(func=tx.func, reverted = tx.hasRevert(), violated_invs = [inv.__str__() for inv in false_invs])
            count += 1

            if training_ratio is not None:
                if count == int(txreplayer.total_transaction_count*training_ratio):
