...
class PptTopLevel(Ppt):
    ...
    def load(self, tx: Transaction, test_mode: bool):
        result_false_invs = list() 
        if not self.is_valid(tx):
            return result_false_invs
        else:
            myvars = [
            ...
            for slice in self.all_slices:
                ...
                false_invs = slice.addValues(vals=vals, test_mode = test_mode)
                result_false_invs.extend(false_invs)
            return result_false_invs
    ...
    def is_valid(self, tx:Transaction):
        ...
        if self.type in [PptType.ENTER, PptType.EXIT]:
            ...
            if self.func.find(tx.func)!=-1 and tx.func != "":
...
class PptSlice(Ppt):
    ...
    def addValues(self, vals: List, test_mode: bool = False):
        false_invs = list()
        size = len(self.invs)
        ...
        for inv in self.invs:
            if test_mode is True:
                inv = copy.copy(inv)
            if inv.falsify is False and not inv.check(vals):
