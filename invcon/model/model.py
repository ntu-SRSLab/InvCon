            ...
            if size > len(self.varValue):
                    for i in range(len(self.varValue), size):
                        ...
                        if self.item_var_type.varBytes<32:
                            ...
                            self.varValue[-1].varSlot = math.ceil(i * math.ceil(self.item_var_type.varBytes/32))
                            self.varValue[-1].varOffset = 0
                    ...
                    for item in DeepDynamicStorage:
                        if item.implyAndSet(slot, value):
                            ...
                            if item.varBytes>=32:
                                    break 
                if not hit:
                    Slot2VariableMap.pop(slot, None)
        ...
        for variable in self.variables: 
                if isinstance(varInfo, VarInfo):
                    if varInfo.name == variable.varName:
                        return variable
                elif isinstance(varInfo, str):
                    if varInfo == variable.varName:
                        return variable
                else:
                    assert False, f"{varInfo} is not VarInfo or str type"
        # assert False, f"{varInfo} not found" 
        return None
       
    
    def getVarValue(self, varInfo: Any):
        variable =  self.getVarType(varInfo)
        if variable is None:
            return None 
        else:
